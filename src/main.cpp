...
String getStyle() {
  String s = "<style>";
  s += "body{font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif; background:#0a0a0c; color:#e0e0e0; margin:0; padding:20px; display:flex; flex-direction:column; align-items:center; min-height:100vh;}";
  s += ".container{width:100%; max-width:440px; background:rgba(255,255,255,0.02); backdrop-filter:blur(15px); border-radius:35px; padding:35px; box-shadow:0 30px 60px rgba(0,0,0,0.7); border:1px solid rgba(255,255,255,0.08); box-sizing:border-box;}";
  s += "h1{font-size:32px; font-weight:900; margin-bottom:30px; background:linear-gradient(90deg, #ccff00 0%, #00e676 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; text-align:center; letter-spacing:-1px;}";
  s += ".card{background:rgba(255,255,255,0.04); border-radius:24px; padding:22px; margin-bottom:20px; border-left:5px solid #ccff00; transition:transform 0.2s ease;}";
  s += ".card:hover{transform:translateX(5px); background:rgba(255,255,255,0.06);}";
  s += ".label{font-size:11px; text-transform:uppercase; letter-spacing:2px; color:#666; margin-bottom:10px; font-weight:700;}";
  s += ".value{font-size:22px; font-weight:700; color:#fff;}";
  s += ".btn{display:block; width:100%; padding:20px; border-radius:20px; border:none; font-size:16px; font-weight:800; cursor:pointer; transition:all 0.3s cubic-bezier(0.4, 0, 0.2, 1); text-transform:uppercase; letter-spacing:1.5px; margin-top:20px; text-align:center; text-decoration:none; box-sizing:border-box;}";
  s += ".btn-primary{background:linear-gradient(135deg, #ccff00 0%, #00e676 100%); color:#000; box-shadow:0 12px 24px rgba(204,255,0,0.25);}";
  s += ".btn-primary:hover{transform:translateY(-2px); box-shadow:0 15px 30px rgba(204,255,0,0.4);}";
  s += ".btn-primary:active{transform:translateY(1px); opacity:0.9;}";
  s += ".btn-outline{background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.1); color:#999; font-size:13px; margin-top:15px;}";
  s += ".btn-outline:hover{background:rgba(255,255,255,0.08); color:#fff; border-color:#ccff00;}";
  s += "form{width:100%;} input{width:100%; box-sizing:border-box; background:rgba(0,0,0,0.4); border:1px solid #222; border-radius:15px; padding:15px; color:#fff; margin-bottom:18px; font-size:16px; transition:all 0.2s;}";
  s += "input:focus{outline:none; border-color:#ccff00; background:rgba(0,0,0,0.6);}";
  s += ".section-title{font-size:13px; font-weight:800; color:#ccff00; margin:35px 0 20px 0; border-bottom:1px solid #222; padding-bottom:8px; letter-spacing:1px;}";
  s += ".status-dot{display:inline-block; width:12px; height:12px; border-radius:50%; margin-right:10px; vertical-align:middle;}";
  s += ".status-online{background:#ccff00; box-shadow:0 0 15px #ccff00;}";
  s += ".status-offline{background:#ff3d00; box-shadow:0 0 15px #ff3d00;}";
  s += "</style>";
  return s;
}
